...
public class MaxHeap<T> {
    ...
    public T deleteMax() {
        ...
        int tempPntr = 1;
        while (tempPntr < pntr) {
            if ((hasLeftChild(tempPntr) && array[tempPntr * 2].key > array[tempPntr].key)
                    && (hasRightChild(tempPntr) && array[tempPntr * 2 + 1].key > array[tempPntr].key)) {
    ...
    static int[] heapSort(int[] array) {
        ...
        for (int i = 0; i < array.length; i++) {
            array[array.length - i - 1] = heap.deleteMax();
        }
        return array;
    }
